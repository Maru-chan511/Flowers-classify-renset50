...
flags.DEFINE_string('checkpoint_path', 
                    '/pretrainedModel/resnet_v1_50.ckpt', 
...
flags.DEFINE_float('num_epochs_per_decay', 3.0,'Number of epochs after which learning rate decays.')
flags.DEFINE_integer('num_samples', 2400, 'Number of samples.')
...
def get_record_dataset(record_path,reader=None, num_samples=50000, num_classes=7):
    if not reader:
...
def get_init_fn():
    ...
    if tf.train.latest_checkpoint(FLAGS.logdir):
        tf.logging.info('Ignoring --checkpoint_path because a checkpoint already exists ' +'in %s' % FLAGS.logdir)
        return None
...
def main(_):
    ...
    dataset = get_record_dataset(FLAGS.record_path, num_samples=num_samples, num_classes=FLAGS.num_classes)
...
#    optimizer = tf.train.AdamOptimizer(learning_rate=0.00001)
    ...
    init_fn = get_init_fn()
    slim.learning.train(train_op=train_op, logdir=FLAGS.logdir, init_fn=init_fn, number_of_steps=FLAGS.num_steps,save_summaries_secs=20,save_interval_secs=600)
