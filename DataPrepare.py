# -*- coding: utf-8 -*-
"""
Created on Fri Oct  9 15:29:39 2020
...
def LabelWrite(path):
    Info=[]
    fileList=["daisy","dandelion","roses","sunflowers","tulips"]
    for i in range(len(fileList)):
        classFile=os.path.join(path,fileList[i])
        img_names = os.listdir(classFile)
        for name in img_names:
            annotations_single_dict=dict()
            ImageName=fileList[i]+"/"+name
            annotations_single_dict.update({"image_id":ImageName})
            annotations_single_dict.update({"flowers_class":int(i)})
            print(annotations_single_dict)
            Info.append(annotations_single_dict)
    json_str=json.dumps(Info)
    with open("E:\\codes\\resnet_fine\\dataSets\\flowers_train_annotations.json","w") as json_file:
        json_file.write(json_str)

def run(): 
    dataset_dir="E:\\codes\\resnet_fine\\dataSets"
